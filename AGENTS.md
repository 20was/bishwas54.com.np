...
- Identity key is `id`; [src/content/sync-manifest.json](src/content/sync-manifest.json) maps ids → routes/paths/body-hashes. Folder moves and renames in the lab do NOT duplicate articles or change URLs. Published routes are frozen; the sync errors on slug/series changes that would alter a URL.
- **Route scheme (since 2026-08-09):** series lessons live at `/tutorials/<series-slug>/<slug>/` (series slug = slugified series name), standalone posts at `/tutorials/<slug>/` — the URL shape tells them apart. Each series gets a landing page `/tutorials/<series-slug>/` listing parts 1..n in order ([src/pages/tutorials/[series]/index.astro](src/pages/tutorials/[series]/index.astro)). Old flat URLs 301 via [public/_redirects](public/_redirects) — append there on any future URL migration, never delete lines.
- **Series contiguity is enforced:** published orders in a series must run 1..n with no gaps or duplicates; the sync fails otherwise (lessons must be published in order).
